...
class CraftStation:
    ...
    def setup(self, recipe):
        return CraftStation(self.crafting_multiplier, recipe)

    def get_craft_rate(self):
        if self.get_recipe().time == 0:
            return float("inf")
        return self.crafting_multiplier / self.get_recipe().time
    ...
    def get_results(self, scale: float = 1):
        return self.get_recipe().get_results() * scale

    def get_results_rates(self, scale: float = 1):
        result_multiplier = scale * self.get_craft_rate()
        return self.get_recipe().get_results() * result_multiplier
